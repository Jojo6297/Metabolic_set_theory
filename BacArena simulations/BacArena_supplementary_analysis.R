...
plot(1:9,cross_share_c[-1,1], pch = forms_mat[-1,1], cex = 2, cex.axis = 1.5, cex.lab = 2, xaxt = "n", col = cols_mat[-1,1], ylim = c(min(cross_share_c[-1,]),max(cross_share_c[-1,])), xlab = "", ylab = "")
...
for(i in 2:10){
  points(1:9,cross_share_c[-1,i], pch = forms_mat[-1,i], col = cols_mat[-1,i], cex = 2)
}
legend(6,-0.05, bty = "n", cex = 2, pch = c(5,1), col = c("darkred", "grey"), legend = c("significant", "non-significant"))
...
interactions <- c("crossfeeding", "shared feeding")
int <- 2
interaction <- interactions[int]
...
pdf(paste("Correlation_metabolic_measures_cooccurrances",interaction,".pdf",sep=""), paper = "a4r", width = 10)

par(mar = c(5.1, 5.7, 4.4, 3.6), xpd = TRUE)

plot(1:9,cross_share_c[-1,1], cex = 2, cex.axis = 1.5, cex.lab = 2, xaxt = "n", col = cols_mat[-1,1], ylim = c(min(cross_share_c[-1,]),max(cross_share_c[-1,])), xlab = "", ylab = "")
axis(1, at = 1:9, labels = as.character(seq(0.1,0.9,0.1)), cex.axis = 1.55)
title(ylab = paste("correlation with ",interaction,sep=""), cex.lab = 2, line = 4)
title(xlab = "richness level", cex.lab = 2, line = 4)
for(i in 2:10){
  points(1:9,cross_share_c[-1,i], col = cols_mat[-1,i], pch = forms_mat[-1,i], cex = 2)
}
legend("topright", inset=c(-0.1, -0.225), bty = "n", cex = 2, pch = c(5,1), col = c("darkgreen", "grey"), legend = c("significant", "non-significant"))
