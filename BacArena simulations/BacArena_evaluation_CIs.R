...
# - the competitive and non-network-based synergistic indices 
# of all pairs of species (sorted by random sets in the folders "random_set_indices_[]")

# This script produces 
# - the Rdata files "share_matrix_set_[]_rn_[].Rdata" and "fracs_matrix_set_[]_rn_[].Rdata"
# - the csv files "Correlations_means/sds_shared/cross_feeding.csv"
# - the Rdata files "cross_share_p.Rdata" and "cross_share_c.Rdata"
# - Supplementary Figure 3a of the manuscript (Scatterplots of cross-feeding and shared feeding)
    ...
    for(rn in 1:10){
      ...
      for(a in 1:(nb-1)){
        for(b in (a+1):nb){
          ...
          for(sp in 1:2){
            if(length(flux[[sp]])>0){
              if(length(flux[[sp]][which(time[[sp]] %in% window[1]:stop)])>0){
      ...
      if(length(list.dirs(dirs_temp[dt]))==1){
        ...
        for(li in 1:length(files)){
          ...
          for(rn in 1:10){
            ...
            save(share_matrix, file = paste("share_matrix_set",set_nr,"_rn",rn,".Rdata",sep=""))
            ...
            #Correlate cross-feeding and shared feeding
            
            # pdf(file = paste("Correlation_cross_feeding_shared_feeding_scatterplot_rn_",rn,"_set_",set_nr,".pdf",sep=""), paper = "a4r", width = 10)
            # par(mar = c(5.1, 5.7, 4.1, 2.1))
            # plot(share_vector_temp, fracs_vector_temp, cex.main = 2, main = paste("set ",set_nr, "\n richness level ", richness[rn], sep=""), cex.axis = 1.5, cex.lab = 2, xlab = "", ylab = "")
            # title(ylab = "crossfeeding", cex.lab = 2, line = 4)
            # title(xlab = "shared feeding", cex.lab = 2, line = 4)
            # dev.off()
            
            cor_sp <- cor.test(as.matrix(share_vector_temp),as.matrix(fracs_vector_temp),method = "spearman")
            cor_pear <- cor.test(as.matrix(share_vector_temp),as.matrix(fracs_vector_temp),method = "pearson")
            ...
            if(mc>0){
              ...
              plot(fracs_vector_temp, index_vector_temp)
              plot(share_vector_temp, index_vector_temp)
        ...
        for(li in 1:length(files)){
          ...
          for(j in 1:4){
            ...
            if(j==4 && d==1){ 
              indices[[j]] <- c(indices_temp, "Curr0.03")
            }else{
...
save(cross_share_p, file = "cross_share_p.Rdata")
save(cross_share_c, file = "cross_share_c.Rdata")
...
write.csv(table, file = "Correlations_means_cross_feeding.csv")
...
write.csv(table, file = "Correlations_sds_cross_feeding.csv")
...
write.csv(table, file = "Correlations_means_shared_feeding.csv")


table <- as.matrix(colSds(results_all_share))
...
write.csv(table, file = "Correlations_sds_shared_feeding.csv")
